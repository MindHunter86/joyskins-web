...
class DuelController extends Controller
{
    ...
    public function viewRoom(){
        ...
        $duel_bets = duel_bets::where('game_id',$id)->where('status',duel_bet::STATUS_WAIT_TO_ACCEPT)->orWhere('status',duel_bet::STATUS_WAIT_TO_SENT)->orWhere('status',duel_bet::STATUS_ACCEPTED)->get();
        return [
            'success'=>true,
        ...
        if($bets == 1) {
            ...
            if(count($bets)==2) {
                ...
                if($duel->status == duel::STATUS_PLAYING) {
                    ...
                    $second = json_decode(\Request::get('items'));
